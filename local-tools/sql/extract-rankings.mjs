...
function usage() {
  return [
    "Usage:",
    "  npm run migration:extract:rankings -- --run-id <id> " +
...
if (database.toLowerCase() !== EXPECTED_DATABASE) {
  throw new Error(
    `Refusing ranking extraction: SQL_DATABASE must be exactly ${EXPECTED_DATABASE}`,
  );
}
...
const connectionConfig = {
  ...
  options: {
    appName: "bbpc-convex-local-ranking-extractor",
    encrypt: true,
    readOnlyIntent: true,
...
try {
  ...
  if (
    ...
    throw new Error(
      `Refusing ranking extraction: connected database must be exactly ${EXPECTED_DATABASE}`,
    );
  }
  listTypeRows = (
...
const outputs = [
  {
    table: "migrationRawRankedListTypes",
    records: listTypeRows.map((row) =>
      transformRankedListTypeRow(runId, row),
    ),
  },
  {
    table: "migrationRawRankedLists",
    records: listRows.map((row) =>
      transformRankedListRow(runId, row),
    ),
  },
  {
    table: "migrationRawRankedItems",
    records: itemRows.map((row) =>
      transformRankedItemRow(runId, row),
    ),
  },
].map((output) => {
...
const finalDirectory = path.join(runDirectory, "rankings");
if (fs.existsSync(finalDirectory)) {
  throw new Error(
    "Refusing to overwrite an existing immutable ranking extract",
  );
}
...
const temporaryDirectory = fs.mkdtempSync(
  path.join(runDirectory, ".rankings-"),
);
try {
