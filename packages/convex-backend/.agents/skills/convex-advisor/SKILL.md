...
4. EMIT findings per specs/finding.schema.json: class perf/correctness/cost, severity from the insight kind (limit hits = high, thresholds = med, retried = med, permanent OCC failure = high), locus {kind: deployment, functionId, tableName}, evidence {kind: insight-event, detail: the bounded event evidence}, fixCapability + autofixable where the repair is mechanical. The runtime event is confirmed evidence that the symptom occurred, but set finding confidence to `confirmed` only after verifying the flagged function and table-access mechanism in current code. Otherwise emit a plausible candidate or pointer finding that does not claim a root cause.
...
- Emit on the findings bus (specs/finding.schema.json). Runtime events are confirmed occurrences; causal findings are `confirmed` only after the implicated code path and table access are verified. Unverified symptoms stay plausible candidates or pointers.
