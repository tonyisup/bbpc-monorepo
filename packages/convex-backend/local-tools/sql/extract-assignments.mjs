...
function usage() {
  return [
    "Usage:",
    "  npm run migration:extract:assignments -- --run-id <id> " +
...
if (database.toLowerCase() !== EXPECTED_DATABASE) {
  throw new Error(
    `Refusing assignment extraction: SQL_DATABASE must be exactly ${EXPECTED_DATABASE}`,
  );
}
...
const connectionConfig = {
  ...
  options: {
    appName: "bbpc-convex-local-assignment-extractor",
    encrypt: true,
    readOnlyIntent: true,
...
try {
  ...
  if (
    ...
    throw new Error(
      `Refusing assignment extraction: connected database must be exactly ${EXPECTED_DATABASE}`,
    );
  }
  assignmentRows = (
    await transaction.request().query(`
      SELECT id, slug, userId, episodeId, movieId, type, playable
      FROM dbo.Assignment
      ORDER BY id;
    `)
  ).recordset;
  audioRows = (
    await transaction.request().query(`
      SELECT id, url, createdAt, userId, assignmentId, fileKey
      FROM dbo.AudioMessage
      ORDER BY id;
    `)
  ).recordset;
  pointLinkRows = (
    await transaction.request().query(`
      SELECT id, assignmentId, userId, pointsId
      FROM dbo.AssignmentPoints
      ORDER BY id;
    `)
  ).recordset;
  syllabusRows = (
    await transaction.request().query(`
      SELECT id, userId, movieId, [order], createdAt, assignmentId, notes
      FROM dbo.Syllabus
      ORDER BY id;
    `)
  ).recordset;
...
const outputs = [
  {
    table: "migrationRawAssignments",
    fileName: "migrationRawAssignments.jsonl",
    records: assignmentRows.map((row) =>
      transformAssignmentRow(runId, row),
    ),
  },
  {
    table: "migrationRawAssignmentAudioMessages",
    fileName: "migrationRawAssignmentAudioMessages.jsonl",
    records: audioRows.map((row) =>
      transformAssignmentAudioMessageRow(runId, row),
    ),
  },
  {
    table: "migrationRawAssignmentPointLinks",
    fileName: "migrationRawAssignmentPointLinks.jsonl",
    records: pointLinkRows.map((row) =>
      transformAssignmentPointLinkRow(runId, row),
    ),
  },
  {
    table: "migrationRawSyllabusEntries",
    fileName: "migrationRawSyllabusEntries.jsonl",
    records: syllabusRows.map((row) =>
      transformSyllabusEntryRow(runId, row),
    ),
  },
].map((output) => {
...
const finalDirectory = path.join(runDirectory, "assignments");
if (fs.existsSync(finalDirectory)) {
  throw new Error(
    "Refusing to overwrite an existing immutable assignment extract",
  );
}
...
const temporaryDirectory = fs.mkdtempSync(
  path.join(runDirectory, ".assignments-"),
);
try {
