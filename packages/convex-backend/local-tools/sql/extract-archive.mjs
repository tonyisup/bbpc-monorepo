...
function usage() {
  return [
    "Usage:",
    "  npm run migration:extract:archive -- --run-id <id> " +
...
if (database.toLowerCase() !== EXPECTED_DATABASE) {
  throw new Error(
    `Refusing archive extraction: SQL_DATABASE must be exactly ${EXPECTED_DATABASE}`,
  );
}
...
const connectionConfig = {
  ...
  options: {
    appName: "bbpc-convex-local-archive-extractor",
    encrypt: true,
    readOnlyIntent: true,
...
try {
  ...
  if (
    ...
    throw new Error(
      `Refusing archive extraction: connected database must be exactly ${EXPECTED_DATABASE}`,
    );
  }
  postRows = (
...
const records = postRows.map((row) =>
  transformArchivePostRow(runId, row),
);
const fileName = "migrationRawArchivePosts.jsonl";
const contents = serializeJsonLines(records);
...
const finalDirectory = path.join(runDirectory, "archive");
if (fs.existsSync(finalDirectory)) {
  throw new Error(
    "Refusing to overwrite an existing immutable archive extract",
  );
}
...
const temporaryDirectory = fs.mkdtempSync(
  path.join(runDirectory, ".archive-"),
);
try {
  ...
  const manifest = {
    ...
    files: [
      {
        table: "migrationRawArchivePosts",
        fileName,
...
process.stdout.write(
  [
    ...
    `runId=${runId}`,
    `migrationRawArchivePosts.rows=${records.length} sha256=${checksum}`,
