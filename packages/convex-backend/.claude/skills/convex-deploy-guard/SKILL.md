...
3. PROD needs a FRESH explicit yes immediately before each action. Then run `pnpm --filter @tonyisup/bbpc-convex-api run deploy:environment:check` and `pnpm --filter @tonyisup/bbpc-convex-api run deploy:production:authorization:check` immediately before the production command; these validate the exact environment, approval, commit, deploy key, and worktree. Use the repository-pinned CLI (`pnpm --filter @tonyisup/bbpc-convex-api exec convex ...`) for deploy, run, env, import, and export commands. A yes given earlier or for another target does not carry.
...
7. Ambiguity = stop: if you cannot determine which deployment a command will hit, use the status tool or compare non-secret environment names with `pnpm --filter @tonyisup/bbpc-convex-api exec convex env list --names-only --deployment <target>` — never guess or display values.
...
- Production actions additionally require both repository checkers immediately before the pinned-CLI command; never substitute a bare `npx convex` invocation.
