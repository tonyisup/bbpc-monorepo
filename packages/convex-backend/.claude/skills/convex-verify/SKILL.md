...
2. SET UP `convex-test`: ensure `convex-test` + `vitest` are dev deps AND a `vitest.config.ts` sets `test.environment: "edge-runtime"` with `server.deps.inline: ["convex-test"]`. Also install `@edge-runtime/vm`. In a test file rooted in the Convex functions directory define `const modules = import.meta.glob("./**/*.ts")`, then call `convexTest(schema, modules)` for the `t` handle. Reuse the project's existing setup.
