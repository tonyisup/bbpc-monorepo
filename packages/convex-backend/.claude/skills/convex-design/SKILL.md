...
- SELF-VERIFY RULE — run the repository typecheck. A local-anonymous push may use the pinned CLI and is exempt from deployment environment checks. Before any cloud push, invoke convex-deploy-guard to identify and announce the exact target. For staging, require the exact staging deploy-key target, confirm production writes are disabled, and run `pnpm --filter @tonyisup/bbpc-convex-api run deploy:environment:check` with the expected staging environment. Production verification is reserved for the approved production runbook, fresh consent, and both production guard checks. Fix every reported error before finishing.
