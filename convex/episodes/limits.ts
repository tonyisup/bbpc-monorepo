...
export function validateBangerPageSize(numItems: number): void {
  if (
    !Number.isSafeInteger(numItems) ||
    numItems < 1 ||
    numItems > MAX_BANGER_PAGE_SIZE
  ) {
    domainError(
      "VALIDATION_FAILED",
      `Banger page size must be an integer from 1 through ${String(MAX_BANGER_PAGE_SIZE)}.`,
      { details: { limit: MAX_BANGER_PAGE_SIZE } },
    );
  }
}
